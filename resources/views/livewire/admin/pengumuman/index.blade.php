...
<div class="card">
    ...
    <div class="card-body">
        ...
        {{-- Tombol tambah --}}
        <div class="mb-3">
            <a href="{{ route('admin.pengumuman.form-p') }}" class="btn btn-primary">
        ...
        <table class="table table-bordered table-hover datatable-basic">
            ...
            <tbody>
                @forelse ($pengumumen as $index => $item)
                    <tr>
                        <td>{{ ($pengumumen->currentPage() - 1) * $pengumumen->perPage() + $index + 1 }}</td>
                        <td>{{ $item->judul }}</td>
    ...
    <div class="datatable-footer d-flex justify-content-between align-items-center px-3 pb-3">
        <div class="dataTables_info">
            Showing {{ $pengumumen->firstItem() }} to {{ $pengumumen->lastItem() }} of {{ $pengumumen->total() }}
            entries
        </div>
        <div class="dataTables_paginate paging_simple_numbers">
            {{ $pengumumen->links('vendor.pagination.bootstrap-4') }}
        </div>
    </div>
    @push('styles')
        <style>
